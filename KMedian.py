...
class FuzzyKMeansM(KMeans):
    ...
    def _e_step(self, X):
        manhattan_distance = manhattan_distances(X, self.cluster_centers_)
        manhattan_distance = np.array(manhattan_distance)
        D = 1.0 / manhattan_distance.argmin(axis=1)

        D **= 1.0 / (self.m - 1)
        D /= np.sum(D, axis=0)[:, np.newaxis]
        # shape: n_samples x k
        self.fuzzy_labels_ = D
        self.labels_ = self.fuzzy_labels_.argmax(axis=0)

    def _m_step(self, X):
        weights = self.fuzzy_labels_ ** self.m
        # shape: n_clusters x n_features
        self.cluster_centers_ = np.dot(X.T, weights).T
        self.cluster_centers_ /= weights.sum(axis=0)[:, np.newaxis]   # [:, np.newaxis]
    ...
    def fit(self, X, y=None):
        ...
        random_state = check_random_state(self.random_state)
        self.fuzzy_labels_ = random_state.rand(n_samples, self.k)
        self.fuzzy_labels_ /= self.fuzzy_labels_.sum(axis=0)    # [:, np.newaxis]
        self._m_step(X)
